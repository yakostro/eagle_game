...
@export var min_segment_duration: float = 3.0   # Minimum segment length
@export var max_segment_duration: float = 8.0   # Maximum segment length
...
func _play_additional_wind_segment():
	...
	if max_start_time <= 0:
		...
		additional_wind.seek(0.0)
		segment_duration = additional_wind_audio_length
	else:
		# Pick random start point
		var start_time = randf_range(0.0, max_start_time)
		additional_wind.seek(start_time)
	
	# Start playing with fade in
	additional_wind.volume_db = -80.0  # Start silent
	...
	fade_tween.tween_delay(segment_duration - (fade_duration * 2))  # Play time minus fade times
	...
	print("Playing additional wind segment: ", segment_duration, " seconds")
